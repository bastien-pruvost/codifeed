...
function Input({ type, startIcon, endIcon, className, ...props }: InputProps) {
  ...
  return (
    <div className="relative">
      ...
      />
      {StartIcon && (
        <StartIcon className="absolute top-1/2 left-2 -translate-y-1/2" />
      )}
      {EndIcon && (
        <EndIcon className="absolute top-1/2 right-2 -translate-y-1/2" />
      )}
    </div>
  )
}
